...
namespace pen_stabilizer {
inline constexpr std::string_view package_version = "0.1.0";
inline constexpr unsigned algorithm_revision = 1;
struct Point {
    double x = 0, y = 0;
...
struct Settings {
    ...
    double cap = 4;
    bool valid() const {
        return std::isfinite(radius) && radius > 0 && radius <= 100 &&
            std::isfinite(window) && window > 0 && window <= 1 &&
            std::isfinite(cap) && cap >= 0 && cap <= 50;
    }
};
struct Sample {
    Point position;
...
public:
    void reset(Settings settings = {}) {
        if (!settings.valid()) throw std::invalid_argument("Invalid pen stabilization settings");
        options = settings;
    ...
    bool append(Sample sample, bool continuous = true) {
        changed = output.size();
        if (closed || !sample.position.finite() || !std::isfinite(sample.time) ||
    ...
    void finish() { changed = mutableBegin = source.size(); closed = true; }
    void cancel() { finish(); }
    bool isClosed() const { return closed; }
    std::size_t committedPrefix() const { return mutableBegin; }
