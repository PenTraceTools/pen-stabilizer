...
int main() {
    try {
        ...
        const auto beforeFinish = s.positions();
        s.finish();
        require(s.isClosed() && s.committedPrefix()==s.positions().size(),"finish contract");
        require(!s.append({{3,4},1,.2f}),"append after finish");
        require(s.positions().size()==beforeFinish.size(),"finish added point");
        s.reset();
        s.append({{1,1},0,.2f});
        s.cancel();
        require(s.isClosed() && s.positions().size()==1,"cancel contract");
        bool rejected=false;
        try { s.reset({true,std::numeric_limits<double>::quiet_NaN(),.12,4}); }
